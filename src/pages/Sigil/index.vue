<template>
  <div class="container">
    <div class="editor">
      <label>Canvas H: {{canvasSize}}</label><br>
      <input type="range" min="100" max="3000" v-model="canvasSize"/>
      <br>
      <label>offset Y: {{offsetY}}</label> <br>
      <input type="range" min="-100" max="100" v-model="offsetY"/>
      <br>
      Show ref image: <input type="checkbox" v-model="showRef"/>
      <br> 
      Spin: <input type="checkbox" v-model="isSpin"/>
      <br> <br>
      <button @click="download"> Download </button>
    </div>

    <div class="ref-img">
      <img draggable="false" v-show="showRef" :src="refImg"/>
    </div>

    <div class="svg-view">
      <!-- v-for="i in 2" :style="{transform: `rotate(${180*i}deg)`, position: 'absolute'}" -->
      <svg 
        class="sigil" ref="downloadRef" v-bind="isSpin?{spin:true}:{}"
        style="--sigilColor: rgb(80, 255, 249); height:100%; width:auto; overflow: visible"
        fill="none" stroke="var(--sigilColor)" version="1.1" :viewBox="viewbox" xmlns="http://www.w3.org/2000/svg"
      >
        <v-style type="text/css">
          @font-face {
            font-family: "PSO";
            src: url("data:application/font-woff;base64,") format("woff"); 
          }

          @font-face {
            font-family: "PSO2";
            src: url("data:application/font-woff;base64,") format("woff"); 
          } 

          .sigil .inner-text { stroke: none; font-family: PSO; fill: var(--sigilColor) }
          .sigil .outer-text { stroke: none; font-family: PSO2; font-size: 321%; fill: var(--sigilColor) }

           
          .sigil[spin] .sigil-frame { animation: spin 40s linear infinite }
          .sigil[spin] .sigil-inner-text { animation: spin 40s linear infinite reverse }
          .sigil[spin] .sigil-spinners > circle {
            transform-origin: center; transform-box: fill-box;
            animation: spin 40s linear infinite;
          }

          @keyframes spin {
            to {
              transform: rotate(-360deg);
            }
          }
        </v-style>

        <g class="sigil-frame">
          <path
            class="sigil-main-frame"
            color="#000000" fill="var(--sigilColor)" stroke="none" transform="translate(-500,-500)" 
            d="m500 55.8c-110.9 0-217.5 41.48-299.1 115.8-16.83-7.503-35.46-11.68-55.07-11.68-74.8 0-135.5 60.75-135.5 135.6 0 45.01 22 84.93 55.82 109.6-6.769 30.94-10.27 62.75-10.27 94.91 0 117.8 46.8 230.8 130.1 314.1 50.65 50.65 112.3 87.8 179.3 109.1 7.127 68.12 64.81 121.3 134.8 121.3s127.7-53.19 134.8-121.3c67.01-21.34 128.6-58.49 179.3-109.1 83.3-83.3 130.1-196.3 130.1-314.1 0-32.17-3.499-63.97-10.27-94.91 33.82-24.66 55.82-64.58 55.82-109.6 0-74.8-60.75-135.6-135.5-135.6-19.6 0-38.23 4.179-55.07 11.68-81.61-74.35-188.2-115.8-299.1-115.8zm0 11.4c106.6 0 209.1 39.32 288.2 109.9-4.375 2.447-8.605 5.123-12.66 8.023-76.03-66.54-173.9-103.5-275.5-103.5s-199.5 37-275.5 103.5c-4.059-2.901-8.289-5.577-12.66-8.023 79.09-70.6 181.6-109.9 288.2-109.9zm0 20.6c99.72 0 195.7 36.14 270.5 101.1-17.01 13.39-30.74 30.76-39.78 50.71-56.26-49.89-128.8-81.85-208.6-86.86-1.88-10.49-11.08-18.49-22.09-18.49s-20.21 8.006-22.09 18.49c-79.82 5.007-152.3 36.97-208.6 86.86-9.041-19.94-22.77-37.32-39.78-50.71 74.76-65.01 170.7-101.1 270.5-101.1zm0 52.6c9.066 0 16.35 7.284 16.35 16.35s-7.284 16.35-16.35 16.35-16.35-7.284-16.35-16.35 7.284-16.35 16.35-16.35zm-21.79 21.71c0.9314 3.774 2.82 7.176 5.395 9.934v75.26h-209c54.79-48.9 125.6-80.24 203.6-85.19zm43.59 0c78 4.953 148.8 36.29 203.6 85.19h-209v-75.26c2.574-2.758 4.463-6.16 5.395-9.934zm-376 7.939c69.34 0 125.5 56.11 125.5 125.4s-56.11 125.4-125.5 125.4c-69.34 0-125.4-56.11-125.4-125.4s56.11-125.4 125.4-125.4zm708.4 0c69.34 0 125.4 56.11 125.4 125.4s-56.11 125.4-125.4 125.4-125.5-56.11-125.5-125.4 56.11-125.4 125.5-125.4zm-708.4 34.5c-50.21 0-90.95 40.74-90.95 90.95s40.74 90.95 90.95 90.95c50.21 0 90.95-40.74 90.95-90.95s-40.74-90.95-90.95-90.95zm708.4 0c-50.21 0-90.95 40.74-90.95 90.95s40.74 90.95 90.95 90.95c50.21 0 90.95-40.74 90.95-90.95s-40.74-90.95-90.95-90.95zm-708.4 3.898c48.1-1e-5 87.05 38.95 87.05 87.05s-38.95 87.05-87.05 87.05c-48.1 0-87.05-38.95-87.05-87.05s38.95-87.05 87.05-87.05zm708.4 0c48.1 0 87.05 38.95 87.05 87.05s-38.95 87.05-87.05 87.05c-48.1 1e-5 -87.05-38.95-87.05-87.05s38.95-87.05 87.05-87.05zm-126.5 38.24c-0.0626 0.1617-0.1236 0.3243-0.1856 0.4863 0.0618-0.1621 0.1231-0.3246 0.1856-0.4863zm-1.344 3.637c-0.0528 0.1491-0.1059 0.298-0.1582 0.4473 0.0522-0.1493 0.1055-0.2982 0.1582-0.4473zm-451.4 3.973h450.1c-4.149 13-6.396 26.84-6.396 41.2 0 20.78 4.692 40.46 13.06 58.07l-18.09 10.45 10.95 18.96 18.12-10.46c19.22 27.76 48.54 48.01 82.62 55.42l-225.1 389.8c-22.5-24.71-54-41.07-89.28-43.9v-20.89h-21.9v20.89c-35.29 2.828-66.78 19.19-89.28 43.9l-225.1-389.8c34.08-7.415 63.39-27.66 82.62-55.42l18.12 10.46 10.95-18.96-18.09-10.45c8.372-17.61 13.06-37.3 13.06-58.07 0-14.36-2.248-28.2-6.396-41.2zm225.1 95.1c-50.65 0-95.52 25.11-122.8 63.54 0.0834-1e-3 0.1644-0.0117 0.248-0.0117 6.629 0 12.37 4.037 14.91 9.766 24.03-33.44 63.27-55.19 107.7-55.19s83.65 21.75 107.7 55.19c2.537-5.728 8.281-9.766 14.91-9.766 0.0836 0 0.1647 0.0104 0.248 0.0117-27.31-38.43-72.18-63.54-122.8-63.54zm-423.7 62.45c4.279 2.564 8.708 4.9 13.28 6.988-5.25 26.54-7.957 53.71-7.957 81.16 0 111 44.08 217.4 122.5 295.9 45.7 45.7 100.9 79.73 160.9 100.2-0.406 4.268-0.623 8.591-0.623 12.96 0 0.6745 0.0155 1.345 0.0254 2.018-63.67-21-122.2-56.69-170.5-105-81.17-81.17-126.8-191.2-126.8-306 0-29.84 3.092-59.37 9.082-88.15zm847.4 0c5.99 28.78 9.082 58.3 9.082 88.15 0 114.8-45.6 224.9-126.8 306-48.3 48.3-106.8 83.99-170.5 105 0.01-0.6722 0.0254-1.343 0.0254-2.018 0-4.373-0.217-8.697-0.623-12.96 60.04-20.46 115.2-54.48 160.9-100.2 78.46-78.46 122.5-184.9 122.5-295.9 0-27.45-2.707-54.62-7.957-81.16 4.567-2.088 8.997-4.424 13.28-6.988zm-546.4 5.572a11.8 11.8 0 0 0-11.75 11.8 11.8 11.8 0 0 0 11.8 11.8 11.8 11.8 0 0 0 11.8-11.8 11.8 11.8 0 0 0-11.8-11.8 11.8 11.8 0 0 0-0.0488 0zm245.2 0a11.8 11.8 0 0 0-11.75 11.8 11.8 11.8 0 0 0 11.8 11.8 11.8 11.8 0 0 0 11.8-11.8 11.8 11.8 0 0 0-11.8-11.8 11.8 11.8 0 0 0-0.0488 0zm-527.2 3.914c15.59 6.259 32.6 9.711 50.41 9.711 4.484 0 8.916-0.2239 13.29-0.6504-4.572 22.49-6.984 45.76-6.984 69.6 0 55.49 13.02 108 36.15 154.5-4.862 4.124-7.959 10.27-7.959 17.12 0 12.36 10.09 22.45 22.45 22.45 2.673 0 5.24-0.4737 7.621-1.338 42.5 63.72 105.5 112.6 179.5 137.3-12.42 17.3-20.87 37.63-24.01 59.66-58.67-20.2-112.6-53.55-157.3-98.26-77.3-77.3-120.7-182.1-120.7-291.5 0-26.6 2.577-52.93 7.582-78.66zm809.2 0c5.005 25.73 7.582 52.06 7.582 78.66 0 109.3-43.43 214.2-120.7 291.5-44.7 44.7-98.63 78.06-157.3 98.26-3.143-22.03-11.59-42.37-24.01-59.66 74.02-24.71 137-73.61 179.5-137.3 2.381 0.8642 4.948 1.338 7.621 1.338 12.36 0 22.45-10.09 22.45-22.45 0-6.848-3.097-12.99-7.959-17.12 23.13-46.56 36.15-99.01 36.15-154.5 0-23.84-2.413-47.11-6.984-69.6 4.373 0.4265 8.805 0.6504 13.29 0.6504 17.81 0 34.82-3.452 50.41-9.711zm-736.2 9.742 104.5 181.1-65.15 37.62c-1.631-0.3791-3.325-0.5859-5.068-0.5859-2.202 0-4.329 0.3242-6.342 0.9199-22.4-45.23-34.99-96.19-34.99-150.1 0-23.62 2.42-46.67 7.018-68.92zm663.2 0c4.598 22.25 7.018 45.3 7.018 68.92 0 53.91-12.6 104.9-34.99 150.1-2.013-0.5957-4.14-0.9199-6.342-0.9199-1.743 0-3.438 0.2068-5.068 0.5859l-65.15-37.62zm-468.4 6.021c-8.834 19.15-13.77 40.45-13.77 62.9 0 78.37 60.12 142.9 136.7 150-1.497-2.461-2.377-5.332-2.377-8.4 0-3.623 1.22-6.972 3.248-9.689-67.14-6.538-119.4-62.98-119.4-131.9 0-19.47 4.179-37.94 11.69-54.58-0.5839 0.0643-1.175 0.1016-1.775 0.1016-6.097 0-11.45-3.416-14.24-8.422zm273.7 0c-2.794 5.006-8.145 8.422-14.24 8.422-0.6 0-1.192-0.0372-1.775-0.1016 7.511 16.64 11.69 35.11 11.69 54.58 0 68.88-52.31 125.3-119.4 131.9 2.028 2.718 3.248 6.067 3.248 9.689 0 3.069-0.88 5.94-2.377 8.4 76.56-7.046 136.7-71.58 136.7-150 0-22.44-4.938-43.75-13.77-62.9zm-136.9 192.6a11.8 11.8 0 0 0-11.75 11.8 11.8 11.8 0 0 0 11.8 11.8 11.8 11.8 0 0 0 11.8-11.8 11.8 11.8 0 0 0-11.8-11.8 11.8 11.8 0 0 0-0.0488 0zm-210.6 10.8 104.6 181.1c-72.48-23.88-134.1-71.57-175.7-133.8 2.749-2.629 4.839-5.944 5.979-9.668zm421.3 0 65.14 37.61c1.14 3.724 3.229 7.039 5.979 9.668-41.57 62.25-103.2 109.9-175.7 133.8zm-507.9 14.72c9.066 0 16.35 7.284 16.35 16.35s-7.284 16.35-16.35 16.35c-9.066 0-16.35-7.284-16.35-16.35s7.284-16.35 16.35-16.35zm594.5 0c9.066 0 16.35 7.284 16.35 16.35s-7.284 16.35-16.35 16.35c-9.066 0-16.35-7.284-16.35-16.35s7.284-16.35 16.35-16.35zm-297.3 128.3c69.34 0 125.4 56.11 125.4 125.4s-56.11 125.4-125.4 125.4-125.4-56.11-125.4-125.4 56.11-125.4 125.4-125.4zm0 34.5c-50.21 0-90.95 40.74-90.95 90.95s40.74 90.95 90.95 90.95 90.95-40.74 90.95-90.95-40.74-90.95-90.95-90.95zm0 3.898c48.1 0 87.05 38.95 87.05 87.05s-38.95 87.05-87.05 87.05-87.05-38.95-87.05-87.05 38.95-87.05 87.05-87.05z" color-rendering="auto" dominant-baseline="auto" image-rendering="auto" shape-rendering="auto" solid-color="#000000" stop-color="#000000" style="font-feature-settings:normal;font-variant-alternates:normal;font-variant-caps:normal;font-variant-east-asian:normal;font-variant-ligatures:normal;font-variant-numeric:normal;font-variant-position:normal;font-variation-settings:normal;inline-size:0;isolation:auto;mix-blend-mode:normal;shape-margin:0;shape-padding:0;text-decoration-color:#000000;text-decoration-line:none;text-decoration-style:solid;text-indent:0;text-orientation:mixed;text-transform:none;white-space:normal"
          />

          <g class="sigil-spinners" fill="none" stroke="var(--sigilColor)" stroke-dasharray="91.9538, 22.0079, 34.9729, 22.0079" :stroke-width="scaleDown(1.475)">
            <circle :cx="scaleDown(35.42)" :cy="scaleDown(-20.45)" :r="scaleDown(10.9)" stroke-dashoffset="-67.97"/>
            <circle :cx="scaleDown(0)" :cy="scaleDown(40.9)" :r="scaleDown(10.9)" stroke-dashoffset="44.97"/>
            <circle :cx="scaleDown(-35.42)" :cy="scaleDown(-20.45)" :r="scaleDown(10.9)" stroke-dashoffset="-10.99"/>
          </g>

          <g class="sigil-frame-text">
            <g class="triangle-text outer-text">
              <g style="transform:rotate(180deg);">
                <text :x="scaleDown(-7.35)" :y="scaleDown(31.1)" style="font-size:139%;" :textLength="scaleDown(4.55)" lengthAdjust="spacingAndGlyphs"> e </text>
                <text :x="scaleDown(3.4)" :y="scaleDown(31.1)" style="font-size:139%;" :textLength="scaleDown(4.55)" lengthAdjust="spacingAndGlyphs"> f </text>
              </g>
              <g style="transform:rotate(60deg);">
                <text :x="scaleDown(-8.1)" :y="scaleDown(30.85)" style="font-size:139%;" :textLength="scaleDown(4.55)" lengthAdjust="spacingAndGlyphs"> m </text>
                <text :x="scaleDown(2.7)" :y="scaleDown(30.85)" style="font-size:139%;" :textLength="scaleDown(4.55)" lengthAdjust="spacingAndGlyphs"> l </text>
              </g>
              <g style="transform:rotate(-60deg);">
                <text :x="scaleDown(-7.55)" :y="scaleDown(30.4)" style="font-size:139%;" :textLength="scaleDown(4.55)" lengthAdjust="spacingAndGlyphs"> p </text>
                <text :x="scaleDown(3.25)" :y="scaleDown(30.4)" style="font-size:139%;" :textLength="scaleDown(4.55)" lengthAdjust="spacingAndGlyphs"> g </text>
              </g>
            </g>

            <g class="outer-text-ring">
              <path
                id="text_3_path" class="text-path text3" stroke-width="0%" fill="none"
                d=" M 0 0 m -369.62, 0 a 369.62,369.62 0 1,1 739.24,0 a 369.62,369.62 0 1,1 -739.24,0 " 
              />
              <g>
                <text class="outer-text circle_text_3a" style="transform:rotate(51.82deg);">
                  <textPath href="#text_3_path" lengthAdjust="spacingAndGlyphs" :textLength="scaleDown(21.3)">
                    farlla
                  </textPath>
                </text>
                <text class="outer-text circle_text_3b" style="transform:rotate(94.56deg);">
                  <textPath href="#text_3_path" lengthAdjust="spacingAndGlyphs" :textLength="scaleDown(21.3)">
                    estlla
                  </textPath>
                </text>
                <text class="outer-text circle_text_3c" style="transform:rotate(-25.26deg);">
                  <textPath href="#text_3_path" lengthAdjust="spacingAndGlyphs" :textLength="scaleDown(17.72)">
                    mylla
                  </textPath>
                </text>
                <text class="outer-text circle_text_3c2"
                  :style="{transform: `translateX(${scaleDown(.5)}) rotate(3.7deg)`, fontSize: '250%'}"
                >
                  <textPath href="#text_3_path" lengthAdjust="spacingAndGlyphs" :textLength="scaleDown(3.2)">
                    2
                  </textPath>
                </text>
                <text class="outer-text circle_text_3d" style="transform:rotate(-68deg);">
                  <textPath href="#text_3_path" lengthAdjust="spacingAndGlyphs" :textLength="scaleDown(21.3)"> 
                    leilla
                  </textPath>
                </text>
                <text class="outer-text circle_text_3e" style="transform:rotate(176deg);">
                  <textPath href="#text_3_path" lengthAdjust="spacingAndGlyphs" :textLength="scaleDown(17.75)">
                    golla
                  </textPath>
                </text>
                <text class="outer-text circle_text_3f" style="transform:rotate(217.27deg);">
                  <textPath href="#text_3_path" lengthAdjust="spacingAndGlyphs" :textLength="scaleDown(17.78)">
                    pilla
                  </textPath>
                </text>
              </g>
            </g>
          </g>
        </g>
        
        <g class="sigil-inner-text" stroke-width="0">
          <path
            id="text_1_path" class="text-path text1" stroke-width="0%" fill="none"
            d=" M 0 0 m -166.26, 0 a 166.26,166.26 0 1,1 332.52,0 a 166.26,166.26 0 1,1 -332.52,0 "
          />
          <text class="inner-text circle_text_1" style="font-size: 166%; transform: rotate(-83.47deg);">
            <textPath href="#text_1_path" startOffset="0.01%" lengthAdjust="spacingAndGlyphs" :textLength="scaleDown(100.820064)"> 
              PLEASE PROTECT US BY VIRTUE OF YOUR THE GREAT LIGHT POWER
            </textPath>
          </text>

          <path 
            id="text_2_path" class="text-path text2" stroke-width="0%" fill="none"
            d=" M 0 0 m -205.16, 0 a 205.16,205.16 0 1,1 410.32,0 a 205.16,205.16 0 1,1 -410.32,0 " 
          />
          <text class="inner-text circle_text_2" style="font-size: 245%; transform: rotate(98.34deg);">
            <textPath href="#text_2_path" lengthAdjust="spacingAndGlyphs" :textLength="scaleDown(122.99342)"> 
              I WISH IT AT AN ALLIANCE FROM SEVERAL YEARS AGO
            </textPath>
          </text>
        </g>
      </svg>
    </div>
  </div>
</template>

<script setup lang="ts">
  import refImg from '../Editor/PSO.png'
  import {ref} from 'vue'
  let canvasSize = $ref(1090)
  let [offsetX, offsetY] = $([ref(0), ref(0)])
  let isSpin = $ref(true)
  let showRef = $ref(true)
  
  let halfSize = $computed(()=>Math.round(100 * (canvasSize/-2) / 100))
  let viewbox = $computed(()=>`${halfSize + +offsetX} ${halfSize + +offsetY} ${canvasSize} ${canvasSize}`)

  let scaleDown = (n:number) => 1000*n/canvasSize + '%'

  let downloadRef = $ref(null)
  function download() {
    if (!downloadRef) return
    var svgSource = (downloadRef as Element).outerHTML
    var svgDataUri = 'data:image/svg+xml;base64,' + btoa(unescape(encodeURIComponent(svgSource)))

    const a = document.createElement('a')
    a.href = svgDataUri
    a.download = 'sigil.svg'
    document.body.appendChild(a)
    a.click()
    document.body.removeChild(a)
  }
</script>

<style scoped>
  .container {
    height: 100vh;
    width: 100vw;

    display: flex;
    justify-content: center;

    background-color: black;
  }
  .svg-view {
    height: 100vh;
    opacity: .5
  }

  .editor{
    position: absolute;
    top: 0;
    left: 0;
    padding: 20px;

    background-color: grey;
    min-width: 150px;
    z-index: 100;
  }
  .ref-img {
    position: absolute;
    height: 100vh;
    display: flex
  }

  .ref-img > img {
    transform: v-bind("`scale(${1000 / +canvasSize}) translate(-1px, ${42 + -offsetY/1.265}px)`")
  }

</style>